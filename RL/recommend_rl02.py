...
##        print('次數: ',number)
        ...
        change_sql1 = "UPDATE tracks_recfromrlresults SET number_of_rec_times = '{}' where track_id = '{}' and  user_id = '{}'".format(number_times,track_id,user_id)
        try:
            cursor.execute(change_sql1)
        except Exception as e:
            db.rollback()
            print('處理失敗:', e)
        else:
            db.commit()
            print('次數更新成功:', m+1,'--',cursor.rowcount)      

    else:
        number_times = 1
        change_sql2 = "UPDATE tracks_recfromrlresults SET number_of_rec_times = '{}' where track_id = '{}' and  user_id = '{}'".format(number_times,track_id,user_id)
        try:
            cursor.execute(sql_1)
            cursor.execute(sql_2) 
            cursor.execute(sql_3)
            ...
            cursor.execute(sql_10) 
            cursor.execute(sql_11)
            cursor.execute(change_sql2)
        except Exception as e:
            db.rollback()
            print('處理失敗:', e)
        else:
            db.commit()
            print('次數更新為1:', m+1,'--',cursor.rowcount)
