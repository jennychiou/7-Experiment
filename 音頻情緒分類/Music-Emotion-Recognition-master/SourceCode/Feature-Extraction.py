...
def extract_feature(path):
    ...
    for line in file_data:
        ...
        # Reading Song
        songname = path + line
        y, sr = librosa.load(songname, duration=60) #讀取音檔
        S = np.abs(librosa.stft(y)) #短時傅立葉轉換
        
        # Extracting Features
        tempo, beats = librosa.beat.beat_track(y=y, sr=sr) # 估計速度和檢測節拍事件
        ...
        # 轉換特徵Transforming Features
        songname_vector.set_value(id, line)  # song name歌名
        tempo_vector.set_value(id, tempo)  # tempo節奏
        total_beats.set_value(id, sum(beats))  # beats拍子
        ...
        mfcc_mean.set_value(id, np.mean(mfcc))  # mfcc梅爾頻率倒譜係數
    ...
    # Converting Dataframe into CSV and JSON
    feature_set.to_csv('Dataset/Emotion_data.csv')
    feature_set.to_json('Dataset/Emotion_data.json')
