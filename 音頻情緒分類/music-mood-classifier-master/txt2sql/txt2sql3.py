...
with open('txtFile/0414-1.txt','r') as f:
        for line in f:
            titledata.append(line.strip('\n').split(','))
with open('txtFile/0414-2.txt','r') as f:
        for line in f:
...
cur  =  conn.cursor ()
sql = "INSERT INTO songlist(github) (id, song, singer, lyrics, vector) VALUES (%s, %s, %s,%s,%s)" 
for i in range(len(singerdata)):
    titlen = titledata2[i]
    aname = singerdata2[i]
    song = eval(titlen)
