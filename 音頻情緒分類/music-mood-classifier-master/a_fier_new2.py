...
##        predict_song(e1.get(), e2.get(),clf)
        ...
        # 寫入CSV二維表格
        import csv
        with open('outputCSV/yourOutput_finderror.csv', 'w', newline='') as csvFile:
            ...
            for i in range(len(titledata)):
                ...
                aname = singerdata2[i]
                tempo = str(tempoL[i])
                energy = str(energyL[i])
                loudness = str(loudnessL[i])
                danceability = str(danceabilityL[i])
                valence = str(valenceL[i])
                acousticness = str(acousticnessL[i])
                haR =  str(haL[i])
                anR = str(anL[i])
                saR = str(saL[i])
                relR = str(relL[i])
                emotion = str(emotionList[i])
                
                Table = [[titlen,aname,tempo,energy,loudness,danceability,valence,acousticness,\
                          haR,anR,saR,relR,emotion]]
